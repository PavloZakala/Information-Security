...
class KeyExpand:
    ...
    def __init__(self, nb, nk, nr):
        ...
        # from https://github.com/nikoliazekter/InformationSecurity/blob/main/Kalyna/kalyna.py
        # self.mult_table = [[0 for j in range(256)] for i in range(256)]
        # for i in range(256):
        #     for j in range(256):
        #         self.mult_table[i][j] = self.multGF2(np.uint8(i), np.uint8(j))
...
if __name__ == '__main__':
    ...
    for i, r in enumerate(round_keys):
        print(i, bytes2string(r))
    print("\n****\n")
    #  Round key generation: 256-bit key expansion for 256-bit block
    key = KeyExpand(4, 4, 14)
    key_test = string2bytes("000102030405060708090A0B0C0D0E0F"
                            "101112131415161718191A1B1C1D1E1F")
    round_keys = key.expansion(key_test)
    ...
    for i, r in enumerate(round_keys):
        print(i, bytes2string(r))
    print("\n****\n")
    #  Round key generation: 512-bit key expansion for 256-bit block
    key = KeyExpand(4, 8, 18)
    key_test = string2bytes("000102030405060708090A0B0C0D0E0F"
                            "101112131415161718191A1B1C1D1E1F"
    ...
    for i, r in enumerate(round_keys):
        print(i, bytes2string(r))
    print("\n****\n")
    #  Round key generation: 512-bit key expansion for 512-bit block
    key = KeyExpand(8, 8, 18)
    key_test = string2bytes("000102030405060708090A0B0C0D0E0F"
                            "101112131415161718191A1B1C1D1E1F"
                            "202122232425262728292A2B2C2D2E2F"
                            "303132333435363738393A3B3C3D3E3F")
